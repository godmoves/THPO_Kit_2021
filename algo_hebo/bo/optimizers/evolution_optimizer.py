...
class EvolutionOpt:
    ...
    def get_init_pop(self, initial_suggest: pd.DataFrame = None, trust_region: tuple = None) -> np.ndarray:
        ...
        if trust_region is not None:
            tr_lb, tr_ub = trust_region
            sobol_samp = sobol_samp * (tr_ub - tr_lb) + tr_lb
        else:
    ...
    def optimize(self, initial_suggest: pd.DataFrame = None, fix_input: dict = None, trust_region: tuple = None) -> pd.DataFrame:
        self.fix = fix_input
        lb = self.space.opt_lb.numpy()
        ub = self.space.opt_ub.numpy()
        prob = get_problem_from_func(self.pymoo_obj, xl=lb, xu=ub, n_var=len(lb))
        init_pop = self.get_init_pop(initial_suggest, trust_region)
        mutation = self.get_mutation()
        crossover = self.get_crossover()
        if self.num_obj > 1:
            algo = NSGA2(pop_size=self.pop, sampling=init_pop,
                         mutation=mutation, crossover=crossover)
        else:
            algo = GA(pop_size=self.pop, sampling=init_pop,
                      mutation=mutation, crossover=crossover)
        res = minimize(prob, algo, ('n_gen', self.iters), verbose=self.verbose)
