...
from util import from_unit_cube, latin_hypercube, to_unit_cube
...
class Searcher(AbstractSearcher):
    searcher_name = "SpacedecaySearcher"

    def __init__(self, parameters_config, n_iter, n_suggestion, **kwargs):
        AbstractSearcher.__init__(
            self, parameters_config, n_iter, n_suggestion)

        self.api_config = parameters_config
    ...
    def _read_config(self):
        return {
            ...
            'experimental_design': 'latin_hypercube',
    ...
    def _init(self, n_suggestions):
        ...
        if exp_design == 'latin_hypercube':
            ...
            halton_sampler = sampler.Sampler(
                method='halton', api_config=self.api_config, n_points=self.n_init)
    ...
    def _find_split(self, X, y) -> Optional:
        ...
        for _ in range(self.config['kmeans_resplits']):
            ...
            if np.count_nonzero(kmeans_labels == 1) > 0 and np.count_nonzero(kmeans_labels == 0) > 0:
                ...
                if DEBUG:
                    print('MARGIN is', margin, np.count_nonzero(
                        kmeans_labels == 1), np.count_nonzero(kmeans_labels == 0))
    ...
    def _suggest(self, n_suggestions):
        ...
        X_cand = self.turbo._select_candidates(
            X_cand, y_cand)[:n_suggestions, :]
        if DEBUG:
            if X.shape[1] == 3:
    ...
    def suggest(self, suggestion_history, n_suggestions=1):
        if suggestion_history:
            X_observed = [list(s[0].values()) for s in suggestion_history[-n_suggestions:]]
            Y_observed = [-s[1] for s in suggestion_history[-n_suggestions:]]
            self.observe(X_observed, Y_observed)
        ...
        X_suggestions = [dict(zip(self.param_names, x)) for x in X_suggestions]
